<script lang="ts">
  ...
  let queryContent: Content = $state({
    json: {
      where: {},
      limit: 20
    }
  ...
  async function handleExecute() {
    ...
    try {
      ...
      const query = 'text' in queryContent ? JSON.parse(queryContent.text) : queryContent.json
      const data = needsData ? ('text' in dataContent ? JSON.parse(dataContent.text) : dataContent.json) : undefined
